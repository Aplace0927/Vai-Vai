                ...
                default:
                    ...
                    foreach (string noteData in SameMeasureNoteSegment)
                    {
                        ...
                        foreach (string sameTimeNote in sameTimeNotes)
                        {
                            ...
                            NoteList = NoteList.Concat(ParseSingleNote(sameTimeNote, NoteTiming, CurrentMeasureFrequency)).ToList();
                        }
                        NoteTiming += CurrentMeasureFrequency;
                    }
                    break;
...
public class SimaiParser
{
    ...
    public void PrintMusicData()
    {
        ...
        {
            System.Console.WriteLine($"Difficulty {(MusicData.Difficulty)i}: Level {MusicData.LevelInfo[i]?.LevelString ?? "N/A"} - Notes Count: {MusicData.LevelInfo[i]?.NoteList.Count ?? -1}");
        }
    }
    public void PrintNoteData(MusicData.Difficulty difficulty)
    {
        ...
        foreach (Note note in levelData.NoteList)
        {
